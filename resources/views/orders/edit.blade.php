        ...
        <div class="container-fluid">
            <div class="container mt-4">
                ...
                <form action="{{ route('orders.update', $order->id) }}" method="POST" class="needs-validation" novalidate>
                    ...
                    <div id="services-container">
                        ...
                        @if (old('services'))
                            @foreach (old('services') as $index => $oldService)
                                @include('orders._service_item', [
                                    'index' => $index,
                                    'services' => $services,
                                    'selectedServiceId' => $oldService['service_id'],
                    ...
                    <button type="button" id="add-service-btn" class="btn btn-outline-primary btn-sm mb-4">Ajouter un
                        service</button>
                    @error('services')
            ...
            <script>
                document.addEventListener('DOMContentLoaded', function() {
                    let serviceIndex =
                        {{ old('services') ? count(old('services')) : (count($orderServices) > 0 ? count($orderServices) : 1) }};
