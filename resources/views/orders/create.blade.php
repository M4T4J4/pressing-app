...
@section('content')
                    ...
                    <div class="card mb-4">
                        ...
                        <div class="card-body">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="datetime-local" id="pickup_date" name="pickup_date" value="{{ old('pickup_date') }}" class="form-control @error('pickup_date') is-invalid @enderror">
                                @error('pickup_date')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="datetime-local" id="delivery_date" name="delivery_date" value="{{ old('delivery_date') }}" class="form-control @error('delivery_date') is-invalid @enderror">
                                @error('delivery_date')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                    ...
                    <div class="card mb-4">
                        ...
                        <div class="card-body">
                            <div id="services-container">
                                {{-- Affiche les services déjà saisis en cas d'erreur de validation --}}
                                @if (old('services'))
                                    @foreach (old('services') as $index => $oldService)
                                        <div class="row mb-3 align-items-end service-item">
                                            <div class="col-md-6">
                                                ...
                                                <select id="service_{{ $index }}_id" name="services[{{ $index }}][service_id]" class="form-select" required>
                                                    <option value="">Sélectionner un service</option>
                                                    @foreach ($services as $service)
                                                        <option value="{{ $service->id }}" {{ $oldService['service_id'] == $service->id ? 'selected' : '' }}>
                                            ...
                                            <div class="col-md-4">
                                                ...
                                                <input type="number" id="service_{{ $index }}_quantity" name="services[{{ $index }}][quantity]" value="{{ $oldService['quantity'] }}" min="1" class="form-control" required>
                                            </div>
                                    ...
                                    @endforeach
                                @else
                                    {{-- Ajoute une première ligne de service par défaut --}}
                                    <div class="row mb-3 align-items-end service-item">
                                        <div class="col-md-6">
                                            <label for="service_0_id" class="form-label">Service :</label>
                                            <select id="service_0_id" name="services[0][service_id]" class="form-select" required>
                                                <option value="">Sélectionner un service</option>
                                                @foreach ($services as $service)
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            // Convertit la collection de services PHP en objet JSON pour le JS
            const servicesData = @json($services);
            let serviceIndex = {{ old('services') ? count(old('services')) : 1 }};

            // Crée le template HTML pour les options de la liste déroulante
            ...
            function addServiceRow(selectedServiceId = '', quantity = 1) {
                ...
                newRow.innerHTML = `
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="number" id="service_${serviceIndex}_quantity" name="services[${serviceIndex}][quantity]" value="${quantity}" min="1" class="form-control" required>
                    </div>
                    <div class="col-md-2 text-end d-flex align-items-center justify-content-end">
                        <button type="button" class="btn btn-danger remove-service-btn mt-3">Supprimer</button>
                    </div>
                ...
                // Si un service ID est fourni, le pré-sélectionner
                if (selectedServiceId) {
                    newRow.querySelector('select').value = selectedServiceId;
                }
